...
.then(function(response) {
  return response.json();
	...
	for (var i = 0; i < data.data.length; i++) {
			console.log(data.data[i].kampus_name.repleace(' ','_'));
			
    	var list = '<a class="nav-link" href=kampus/"'+data.data[i].kampus_name.repleace(' ','_')+'.html"><img src="/inkam/asset/'+data.data[i].kampus_logo+'"> <span>'+data.data[i].kampus_name+'</span></a>';
