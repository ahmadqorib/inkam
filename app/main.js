...
.then(function(response) {
  return response.json();
	...
	for (var i = 0; i < data.data.length; i++) {
			console.log(data.data.kampus_name);
			
    	var list = '<a class="nav-link" href="'+data.data.kampus_name+'.html"><img src="/inkam/asset/'+data.data[i].kampus_logo+'"> <span>'+data.data[i].kampus_name+'</span></a>';
